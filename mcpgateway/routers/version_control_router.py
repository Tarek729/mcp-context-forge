"""Version Control API router for managing MCP server version approvals."""
...
# Third-Party
from core.version_control_core import ServerVersion, VersionControlCore, VersionControlDB
...
@router.get("/pending", response_model=DeactivatedVersionsResponse, summary="List all pending server versions", description="Returns all server versions with status='pending' awaiting approval")
async def list_pending_versions(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        return DeactivatedVersionsResponse(total=len(versions), versions=versions)

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch pending versions: {str(e)}")


@router.get(
    "/active", response_model=DeactivatedVersionsResponse, summary="List all active server versions", description="Returns all server versions with status='active' from the version control database"
)
async def list_active_versions(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        with vc_core.db.get_vc_session() as session:
            ...
            query = select(ServerVersion).where(ServerVersion.status == "active").order_by(ServerVersion.created_at.desc(), ServerVersion.version_number.desc())
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch active versions: {str(e)}")
...
async def list_deactivated_versions(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch deactivated versions: {str(e)}")
...
async def update_version_status(version_id: str, request: UpdateVersionStatusRequest, current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        updated_version = vc_core.update_version_status(version_id=version_id, new_status=request.new_status, updated_by=user_email)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update version status: {str(e)}")
...
def get_vc_core():
    ...
    vc_db_url = os.getenv("VC_DATABASE_URL", "postgresql+psycopg://postgres:mysecretpassword@localhost:5433/mcp_version_control")

    db_manager = VersionControlDB(main_db_url=main_db_url, vc_db_url=vc_db_url, echo=False)
...
async def check_gateway_changes(gateway_id: str, current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        return CheckChangesResponse(gateway_id=gateway_id, has_changes=has_changes, message=message)

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to check for changes: {str(e)}")
...
async def create_pending_gateway_version(gateway_id: str, current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        vc_core = get_vc_core()
        version = await vc_core.create_pending_version(gateway_id=gateway_id, created_by=user_email)
        ...
        if version:
            ...
            return CreatePendingVersionResponse(success=True, version=version_dict, message=f"Successfully created pending version {version.version_number} for gateway {gateway_id}")
        else:
            return CreatePendingVersionResponse(success=False, version=None, message=f"Failed to create pending version for gateway {gateway_id}")

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to create pending version: {str(e)}")
